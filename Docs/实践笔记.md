...
1. [USTC 编译原理和技术 2023](https://ustc-compiler-principles.github.io/2023/lab3/environment/)
...
什么意思呢？我不知道读者知不知道，但是乍一看我是不知道，所以右键查找所有实现（VSCode下），左侧栏（默认设置）中就会出现一列结果，其中baseline（想打菜狗的但是不知道会不会有点不礼貌，但是仓库名就叫这个名字啊）中的有12个结果，应该比原本baseline（官方赛题提供的仓库）多了`unix`、`tcp`、`udp`（网络相关，可能今年年中的内核赛多了网络的功能？）以及`urandom`（Linux随机数生成设备）。然后大多数这个方法的实现应该都是`todo!();`，要看的应该是`OSInode`内的：
...
可以看到调用了内部inner的`get_all_cache`方法，那么再继续往里找：
...
可以看到第8行调用了`Inode`内部字段`file_content`，这个字段在`Ext4OSInode`和`Ext4Inode`都是没有的，所以再继续往下看看，可以看到通过文件大小获取缓存页数，然后再通过get_single_cache_lock获取指定cache_id的缓存页，加入到缓存列表中。

那么就需要再去看看`get_single_cache_lock`做了什么：
...
整个看下来可以发现，**需要添加file_cache_mgr以及第16行的那个闭包！**

先来说说闭包，参考`get_neighboring_sec`编写那个闭包，首先先写一个`get_neightboring_blk`:
...
impl Ext4OSInode {
    pub fn get_neighboring_blk(&self, inner_cache_id: usize) -> Vec<usize> {
        ...
        for _ in 0..blk_per_cache {
            if blk_id >= blk_cnts {
                println!("[kernel in get_single_cache] blk_id is out of bound, blk_id: {}, blk_cnts: {}", blk_id, blk_cnts);
                break;
            }
            // 获取物理块号
            // 此处有问题
            let start_block_id = self.ext4fs.get_pblock_idx(&inode_ref, blk_id as u32).unwrap();
            block_ids.push(start_block_id as usize);
            blk_id += 1;
        }
        println!("[kernel in get_single_cache] block_ids is {:?}", block_ids);
        block_ids
    }
}
```

**原理有空再写了，现在是23:12（2025/1/4），现在要紧事是赶紧把剩下几个File Trait给实现了，然后进入NPUcore的Bash界面**

然后还可以看到用到了`file_cache_mgr`的`get_cache`方法，回顾一下`Inode`结构体定义：
...
`file_cache_mgr`是一个`PageCacheManager`类型，在每次`fat32`文件系统试图读取一个块的时候，都会先在`PageCacheManager`中寻找，找不到再从磁盘块设备对象中找，显然我还没有在`Ext4FileSystem`实现相同的机制（本身在更换baseline之前其实这一步就得做了，因为对`Nand`对象读`buf`写`buf`不一致，就只能先通过直接放内存上的形式来操作）

综上所述，需要先为`Ext4OSInode`实现`file_cache_mgr`类似的机制。

那么先在Ext4OSInode内部添加一个`file_cache_mgr`字段：
...
// 修改完这里在每一个new构造函数里面或者直接Self的地方做相应的修改就行了，
// 有些是原file_cache_manager的clone()，
// 有些是直接Arc::new<PageCacheManager::new()>
// 判断方法很简单，看是不是全新的Ext4OSInode对象就行了，这里就不仔细贴出来了
```

（写到这里的时候，感觉智能指针是真好用，套层`Arc`就行了，不用再去仔细管，但是还有一个东西要注意，没记错的话好像是被包装的对象非`mut`的情况下，对内部内容直接修改的话，只有`Arc`计数为1的时候才可以进行操作（通过`Arc`的`get_mut`），不过现在还没涉及到，后面要去看看怎么修改，比方说时间修改，不能直接对其内容进行修改，但是现在还不需要用到时间，所以先不管，后面再去完善）

然后来看看`PageCacheManager`的定义：
...
很好，十分的好，缓存池字段直接使用的`Vec`动态数组，不是定长数组，也就是说在不考虑极端情况下，只考虑能进入`Shell`界面进行简单操作的情况下，直接往里灌数据就完事了，读一个就往里塞，读一个就往里塞，反正宿主机我的鸡哥（机械革命）有32G内存，不怕（后面肯定还是得考虑`oom`的事情，现在暂时不用考虑）。

然后来看看`PageCacheManager`的`get_cache`方法（此前`lazy_static`代码块中`FILE_SYSTEM`的创建时，有用到一个`get_block_cache`方法，属于`BufferCache`的，不过两者可能略有不同，但是可能实现思路是一样的，然而在阅读了代码之后，发现`PageCacheManager`的`try_get_cache`压根没被调用）
...
其中第36行的`read_in`里面的实现是连续读两个块（如果neighbor提供给他的块号真的连续的话）

NPUcore中的`fat32`比较抽象，怎么说呢

**一页两个簇，一簇一扇区，一扇2KB**

~~好诗，好诗~~

也就是说一页的大小是**4KB**（缓存页，写到这里的时候内存实现还没去细看，但是后面得去仔细看看，通过这一系列操作（包括读`fs`的实现，以及调用到的别的地方的内容的实现，还有`ext4_rs`的整合），感觉真的挺爽的，像是把一个地方闹了个底朝天，处处留下注释或者把原来的英文注释改成中文注释就像是在这里布满了足迹，就像是把一个游戏里面的所有关卡都遍历了一样爽）

所以我在读其`read_in()`的代码以及`neighbor()`(`fat32`下的)代码的时候思路有那么一会儿被卡住（也可能是因为我每次看代码的周期是一个礼拜导致的）
...
所以在`Ext4OSInode`中，也要做类似的操作，即思路为：
...
能直接这样做吗？**fat32**是使用一个一个簇来存放文件数据，而**ext4**是通过一个一个数据块来存数据，并且如果文件大小比较大的话，还会用到多级表结构来获取数据块（就像`cache`以及多级页表那样）（然而ext4中好像默认是使用`extent tree`而不是多级间接块`indirect block`了），所以现在先使用简单的方法实现，即不考虑大文件情况的实现。

然后需要参考`ext4_rs`中的`read_at`方法，因为这里面有使用到文件的数据块（就像`fat32`下的`get_xxx_cache`需要用到`clus_list`一样）

在使用的时候，发现有个地方（`Ext4FileSystem::get_pblock_idx`）出了问题，属于`ext4_rs`的，不过谢天谢地（也算我运气好（~~有时候会怀疑我运气是不是点满了~~），想到会不会是这个出错了，然后去看看仓库里到底是怎么说的，说不定有别的队伍也在用这个仓库，然后通过阅读`issue`大概了解了是什么原因，只能说还得去整理一下ext4的原理知识，这里涉及到的是`extent`中**物理块号**和**逻辑块号**的关系，现在只是知道个大概，知道怎么调用，等整完了这些东西之后得去整理一下），`ext4_rs`**5**天前修复了这个问题，`issue`是两周前提的（写到这里的时候是**2025.1.4**），然后是**3**天前解决的，什么情况呢，就是读取的物理块号出错了。

还有到目前为止还没有使用`ext4_rs`好好测试一下（因为其默认测试实现生成了一个8G还是10G大小来着的img文件来测试，~~有点逆天，并且我也心疼硬盘~~，生成时间也比较久，然后改了大小之后，不知道为什么不能正常运行，所以就先放下了）

`issue`见此：[`get_pblock_idx()` 应返回 `pblock + lblock` 而不是 `pblock`?#2](https://github.com/yuoo655/ext4_rs/issues/2)
...
就是这里面，输入不同的逻辑块号，输出的是同一个物理块号（`find_extent`函数），按照`issue`中作者（仓库作者）给的解释，应该就是返回的是物理块号的第一块，需要修改：

```diff
--- a/src/ext4_impls/extents.rs
+++ b/src/ext4_impls/extents.rs
@@ -62,7 +62,7 @@ impl Ext4 {
                 index: None,
                 extent: Some(extent),
                 position: pos,
...
也就是说`find_extent`内部`pblock`行需要修改，所以修改就完事了。

引用作者的话：

> pblock的计算方式应该是 **逻辑块号 - extent对应逻辑块的起始块号 + extent对应物理块的起始块号**

然后是`File` Trait的`read`方法，



接着是`File` Trait的 `get_dirent` 方法

##### 2.2.3.2 get_dirent 适配

上面成功进入NPUcore之后，使用`ls`的时候需要使用到这个方法，应该是要获取到目录下的所有目录项，然后再传递回去，这个方法被系统调用`sys_getdents64`所调用:
...
可以看到调用了`file`的`get_dirent`函数，这里`file`是一个实现了`File` Trait的对象，而我们的`Ext4OSInode`还没有实现这个功能，所以需要先去看看`fat32`下是怎么实现的，也就是说先去看看`OSInode`的实现：
...
可以看到这里面用到`inner`的`dirent_info_lock`方法，这里面的`inner`虽然说是一个实现了`InodeTrait`的对象，但实际上是一个fat32的inode对象，现在不需要关注里面是怎么实现的，只需要知道**传入的offset参数是会变的，并且在fat32的文件系统下，这个函数会被执行多次，每次offset都会变，直到最后返回的vec是空的时候就不会再调用了**，同时最后一次两个`offset`打印信息是一样的（为什么我知道呢，因为这是我用调试信息打印试出来的，在第一次直接给`Ext4OSInode`实现这个函数的时候，这个函数就一直被调用，没有停下来过，也就是说应该要有一个边界条件）

也就是说可以猜测`ls`的实现原理，会调用一堆其他的系统调用（包括`fstatat`等等），然后最关键的调用是`sys_getdents64`，当这个调用返回的是空值的时候就会停止调用这个系统调用。关于ls的运行流程后面考虑看要不要补充一下。

所以现在就需要为`Ext4OSInode`实现`get_dirent`这个方法
...
可以看到第26行直接通过`dir_get_entries`方法获取到目录项

但是这样会有问题，因为不论调用多少次，都会返回一个不为空的目录项数组（如果当前文件是个目录的话，因为只要是个目录就会有当前目录和父级目录两个目录项，也就是"`.`"和"`..`"）

所以我们还需要进行修改，利用这个offset来作为已经获取了多少目录项的标记，一开始想的是参照dir_get_entries里面的实现来修改，但是写到这里的时候忽然想到可以通过比较`offset`，因为这里是一次性获取多个目录项，所以第二次调用进入这里面的时候，直接返回空就行了，
