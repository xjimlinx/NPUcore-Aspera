...
这里先讲get_all_caches是怎么做的，因为前面做的工作我不一定想的起来，所以先把现在在做的东西先写一写，效果可能好些，
...
可以看到调用了内部inner的get_all_cache方法，那么再继续往里找：
...
可以看到第8行调用了`Inode`内部字段`file_content`，这个字段在`Ext4OSInode`和`Ext4Inode`都是没有的，所以再继续往下看看，可以看到通过文件大小获取缓存页数，然后再通过get_single_cache_lock获取指定cache_id的缓存页，加入到缓存列表中，所以在`Ext4OSInode`中，也要做类似的操作，即思路为：
...
能直接这样做吗？fat32是使用一个一个簇来存放文件数据，而ext4是通过一个一个数据块来存数据，并且如果文件大小比较大的话，还会用到多级表结构来获取数据块（就像cache以及多级页表那样）所以现在先使用简单的方法实现，即不考虑大大文件情况的实现。
