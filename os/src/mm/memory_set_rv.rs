...
use crate::hal::arch::riscv::rv_pagetable::{PTEFlags, RVPageTable, RVPageTableEntry};
...
impl MemorySet {
    ...
    pub fn create_elf_tables(
        ...
        let mut phys_user_sp = RVPageTable::from_token(self.token())
...
impl MapArea {
    ...
    pub fn map_from_existing_page_table(
        &mut self,
        dst_page_table: &mut RVPageTable,
        src_page_table: &mut RVPageTable,
    ) -> Result<(), ()> {
    ...
    pub fn unmap(&mut self, page_table: &mut RVPageTable) -> Result<(), MemoryError> {
        let mut has_unmapped_page = false;
        for vpn in self.inner.vpn_range {
    ...
    /// If `new_end` is equal to the current end of area, do nothing and return `Ok(())`.
    pub fn shrink_to(&mut self, page_table: &mut RVPageTable, new_end: VirtAddr) -> Result<(), ()> {
        let new_end_vpn: VirtPageNum = new_end.ceil();
        let old_end_vpn = self.inner.vpn_range.get_end();
        if new_end_vpn > old_end_vpn {
            warn!(
    ...
    #[cfg(feature = "oom_handler")]
    pub fn do_oom(&mut self, page_table: &mut RVPageTable) -> usize {
    ...
    #[cfg(feature = "oom_handler")]
    pub fn force_swap(&mut self, page_table: &mut RVPageTable) -> usize {
        let start_vpn = self.inner.vpn_range.get_start();
        let swapped_before = self.inner.swapped;
        warn!("{:?}", self.inner.active);
        while let Some(idx) = self.inner.active.pop_front() {
